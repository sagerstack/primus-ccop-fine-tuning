#!/usr/bin/env python3
"""
Contextualize v2 — gpt-4o-mini with explicit acronym dictionary.

Builds on Exp #14 finding (gpt-4o-mini's loose synonym-heavy style works) +
Exp #39 finding (Claude's domain-precise style hurts retrieval).

Goal: keep loose synonym style + fix the hallucinated acronym expansions
that gpt-4o-mini invented (especially CIIO).

Cost: ~$0.20, ~10 min for 495 chunks.
"""
...
NEW_COLLECTION = "ccop_clauses_contextual_v2"
SOURCE_COLLECTION = "ccop_clauses_contextual"

ACRONYM_DICTIONARY = """
ACRONYM DICTIONARY (USE THESE EXACT EXPANSIONS — never invent alternatives):

  CCoP    = Cybersecurity Code of Practice (the Singapore regulatory document)
  CCoP 2.0 = Cybersecurity Code of Practice for Critical Information Infrastructure, Second Edition
  CSA     = Cyber Security Agency (the Singapore regulator that issues CCoP)
  CIIO    = Critical Information Infrastructure Owner (the regulated entity — the organisation that operates a CII)
  ...
  Commissioner = the Commissioner of Cybersecurity (the regulator's head, who issues directions/waivers)

DO NOT introduce any other expansions or organisation names not in this list.
"""
...
def generate_context(client, model: str, payload: dict) -> str:
    ...
    prompt = CONTEXT_PROMPT.format(
        acronym_dictionary=ACRONYM_DICTIONARY,
...
def main():
    ...
    logger.info(f"Generating contexts via {model_id} (with acronym dictionary)...")
    contexts = {}
    t0 = time.time()
    n_done = 0
    with ThreadPoolExecutor(max_workers=20) as pool:
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                contexts[pid] = ""
                logger.warning(f"err pid={pid}: {e}")
            n_done += 1
            if n_done % 50 == 0:
    ...
    for i, r in enumerate(new_records):
        ...
        new_payload["context_line_v2"] = r["context"]
    ...
    # Save samples
    sample_path = "/Users/sagarpratapsingh/dev/sagerstack/studio-ssdlc/.lab/workspace/contextualize-v2-samples.json"
    sample = []
    for r in new_records[:15]:
        sample.append({
            "citation_id": (r["payload"].get("citation_id")),
            "context": r["context"],
            "original_preview": (r["payload"].get("original_text") or r["payload"].get("text", ""))[:200],
        })
